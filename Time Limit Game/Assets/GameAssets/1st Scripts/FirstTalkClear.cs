...
public class FirstTalkClear : MonoBehaviour
{
    ...
    void Start()
    {
        if (dialogueText == null || playerObject == null || enemyObject == null)
        {
            ...
            if (isFirstTextCompleted)
            {
                ...
                else if (!isTyping)
                {
                    if (currentDialogueIndex >= dialogues.Count)
                    {
                        ...
                        SceneManager.LoadScene("1st Game"); // "NextSceneName" を次のシーン名に変更
                    }
                    else
                    {
                        NextDialogue();
